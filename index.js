...
// ====================================================================
// 🐳 CONFIGURAÇÃO DO TOKEN (Caso a variável da ShardCloud não funcione)
const TOKEN_RESERVA = 'COLE_SEU_TOKEN_AQUI'; 
// ====================================================================

// Função interna que define qual token usar sem precisar de arquivo .env
const obterToken = () => {
    if (process.env.TOKEN && process.env.TOKEN.trim() !== '') {
        return process.env.TOKEN.trim();
    }
    return TOKEN_RESERVA;
};
...
const registarComandos = async (guildId) => {
    ...
    const rest = new REST({ version: '10' }).setToken(tokenAtivo);
