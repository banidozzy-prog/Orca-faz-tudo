...
client.once('ready', async () => {
    ...
    // Registra o comando automaticamente
...
// --- SISTEMA DE CONFIGURAÇÃO (INTERAÇÕES) ---
client.on('interactionCreate', async (interaction) => {
    if (interaction.isChatInputCommand() && interaction.commandName === 'configurar') {
        const menu = new StringSelectMenuBuilder()
            ...
            .addOptions([
                { label: 'Logs de Mensagens', value: 'logs', description: 'Canal para mensagens apagadas/editadas' },
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'menu_config') {
        ...
        await interaction.update({ content: `Selecione o canal para **${escolha === 'logs' ? 'Logs de Mensagens' : 'Boas-vindas'}**:`, components: [row] });
    }
...
client.on('guildMemberAdd', async (member) => {
    ...
    try {
        ...
        if (inviteUsado) logInvite = `📥 **${member.user.username}** foi convidado por **${inviteUsado.inviter.username}**.`;
