...
// --- FUNÇÃO PARA REGISTRAR O COMANDO /CONFIGURAR ---
...
async function executarConfigurar(interaction) {
    ...
    const menuMembros = new ChannelSelectMenuBuilder()
        ...
        .setPlaceholder('Selecione o canal para logs de Membros e Entrada')
        .addChannelTypes(ChannelType.GuildText);
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChannelSelectMenu() || interaction.isRoleSelectMenu()) {
        ...
        if (interaction.customId === 'log_membros') {
            ...
            await interaction.reply({ content: `✅ Canal de logs de **Membros e Entrada** definido com sucesso para <#${interaction.values[0]}>!`, ephemeral: true });
        }
...
// --- EVENTO: ENTRADA DE MEMBRO + EMBED DE BOAS-VINDAS ---
client.on('guildMemberAdd', async member => {
    ...
    try {
        ...
        if (entrouPorVanity) {
            logInvite = `📥 **${member.user.username}** entrou usando o link personalizado do servidor.`;
        } else if (cacheAntigo) {
            ...
            if (inviteUsado) {
                ...
                logInvite = `📥 ${member} foi convidado por **${inviter.username}** (atualmente tem ${totalConvites} invites).`;
            }
        }
    ...
    const embedBoasVindas = new EmbedBuilder()
        ...
        .setDescription(`Olá ${member}, seja muito bem-vindo(a) à nossa comunidade!\n\n📌 Não se esqueça de ler as diretrizes e aproveitar o espaço!`)
        .setColor('#00FF7F')
        .setThumbnail(member.user.displayAvatarURL({ dynamic: true }))
        .setImage('URL_DA_SUA_IMAGEM_AQUI') // Substitua pelo link do banner se quiser, ou apague esta linha
        .setFooter({ text: `Agora somos ${member.guild.memberCount} membros!`, iconURL: member.guild.iconURL() })
        .setTimestamp();

    canalLog.send({ content: logInvite, embeds: [embedBoasVindas] }).catch(() => null);
});
