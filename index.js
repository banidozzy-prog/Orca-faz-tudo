...
const client = new Client({
    ...
    // Partials são obrigatórios para o bot ler mensagens e fotos antigas
...
function temPermissao(interaction) {
    ...
    // Se for o dono do servidor ou tiver a permissão Admin nativa, sempre pode
    if (interaction.user.id === interaction.guild.ownerId) return true;
    if (interaction.member.permissions.has(PermissionFlagsBits.Administrator)) return true;

    // Se tiver o cargo configurado pelo menu, também pode
...
client.once('ready', async () => {
    ...
    // Preenche a memória de convites assim que o bot liga
    for (const guild of client.guilds.cache.values()) {
        await carregarInvitesDoServidor(guild);
    }
});

// Mantém os convites atualizados caso criem ou deletem links com o bot online
...
async function executarConfigurar(interaction) {
    ...
    // Menu para Logs de Entradas/Saídas (Membros e Invites)
    ...
    // Menu para definir o Cargo que pode usar o Bot
...
// --- ESCUTANDO AS INTERAÇÕES (SLASH COMMANDS E MENUS) ---
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChannelSelectMenu() || interaction.isRoleSelectMenu()) {
        ...
        if (interaction.customId === 'log_mensagens') {
            ...
            await interaction.reply({ content: `${EMOJI_CRIAR} Canal de logs de **Mensagens** definido com sucesso para <#${interaction.values[0]}>!`, ephemeral: true });
        }

        if (interaction.customId === 'log_membros') {
            configAtual.membros = interaction.values[0];
            await interaction.reply({ content: `${EMOJI_CRIAR} Canal de logs de **Membros e Invites** definido com sucesso para <#${interaction.values[0]}>!`, ephemeral: true });
        }

        if (interaction.customId === 'cargo_staff') {
            configAtual.cargoStaffId = interaction.values[0];
            await interaction.reply({ content: `${EMOJI_CRIAR} Cargo permitido definido com sucesso para: <@&${interaction.values[0]}>!`, ephemeral: true });
        }
...
// --- EVENTO: MENSAGEM APAGADA (COM SUPORTE A TEXTO E FOTOS) ---
...
// --- EVENTO: ENTRADA DE MEMBRO (ESTILO DE TEXTO DO ORCA/LIDERANÇA) ---
    ...
    try {
        ...
        if (entrouPorVanity) {
            ...
            if (inviteUsado) {
                ...
                // Texto limpo sem caixa de embed igual ao seu print
...
// Proteção para o Token: Puxa direto das variáveis seguras da ShardCloud
